...
void print_poly(struct node *poly)
{
    ...
    bool isFirstTerm = true; // flag to check if it is the first term

    while (poly != NULL)
    {
        if (poly->num == 0) // neu he so = 0 thi bo qua
        {
            poly = poly->next;
            continue;
        }

        if (!isFirstTerm && poly->coeff >= 0) // Add "+" sign before positive coefficients (except for the first term)
        {
            cout << " + ";
        }

        if (poly->num == 0) // neu he so == 0
        {
            cout << abs(poly->coeff); // print the absolute value of the coefficient
        }
        else if (poly->num == 1) // neu he so == 1
        {
            if (abs(poly->coeff) != 1) // print the absolute value of the coefficient if it's not 1
            {
                cout << abs(poly->coeff);
            }
            cout << "x";
