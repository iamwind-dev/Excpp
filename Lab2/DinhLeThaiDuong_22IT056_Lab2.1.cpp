...
//----------------
void Inp(SV *&L){//dinh nghia ham Inp de nhap thong tin sinh vien va luu tru no trong ds lien ket
	...
	while(count<=n){//su dung while de thuc hien viec nhap tt sv cho den so luong sv dat den gia tri n
...
//
void Outp(SV *L){//Dinh nghia ham Outp de xuat thong tin SV
	SV *P;//khai bao mot con tro P de duyet qua dslk
	int count=0;
	P=L;//bat dau ds
	cout << "STT\t\t" << "Name\t\t" << "DOB\t\t" << "Math\t\t" << "Physic\t\t" << "Language"<< endl;
	while(P!=NULL){//su dung while de duyet qua ds lien ket va in ra mh
		count++;
...
void Add(SV *&L) {
    ...
    cin >> P->language;
    P->next = NULL;
    //Thêm nút m?i vào cu?i danh sách liên k?t
    if (L == NULL) {//neu ds rong
        L = P;//them du lieu cua P vao L
    } else {//neu ds k rong
...
int count_student_in_year(SV *&L,int year){
	SV *Q;// Khai bao 1 con tro Q tro den danh sach sinh viên L
	int count = 0;//khoi tao bien dem so sv
	Q = L;//gan con tro Q = dau ds L
	while(Q != NULL){//dung while de duyet ds sv
		if(Q->year == year){// kt nam sinh cua sv co bang voi nam yeu cau k
			count++;//tang count
		}
...
bool has_student(SV *&L,string name){
	SV *Q;// Khai báo 1 con tro Q tro den danh sach sinh viên L
	Q = L;//gan con tro Q = dau ds L
	while(Q!=NULL){//dung while de duyet ds sv
		if(Q->name == name){// kt nam sinh cua sv co giong voi nam yeu cau k
			return true;
		}
		Q = Q->next;//di chuyen con tro Q den sv tiep theo
	}
...
void delete_student(SV *&L,string name){
	...
	if(L == NULL){//kiem tra truong hop ds rong
		return;//ket thuc ham
	}
	if(L->name == name){// kt ten sinh cua sv co bang voi ten yeu cau k
		Q = L;//gan con tro Q = con tro dau tien trong ds L
		L = L->next;// Cap nhat dau danh sách lien ket de tro den nut thu hai
		delete(Q);//giai phong vung nho
		return;
	}
	P = L;//gan con tro P = con tro dau tien trong ds L
	while(P->next != NULL){//duyet ds
		if(P->next->name == name){//Neu ten cua node ke tiep cua node hien tai (P->next->name) trung voi name
...
void print_students_with_math_less_than_5() {
	...
	int count=0;
    Q = L;//gan con tro Q = dau ds L
        while (Q != NULL) {//duyet ds
            if (Q->math < 5){//kiem tra co dieu kien can
